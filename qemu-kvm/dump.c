...
int get_dirty_pages_new()
{
	//return get_dirtypages_speed();
    return ram_list.dirty_pages;
}



char* get_dirty_maps_new()
{
    RAMBlock *block;
    ram_addr_t last = 0;
    QLIST_FOREACH(block, &ram_list.blocks, next)
    ...
    int total = last >> TARGET_PAGE_BITS;
    
    char buffer[4*total + 1];
    int offset = 0;
    int i;
    for(i = 0; i < total; i++)
    {
...
int fd_write_dirty(int fd, void *buf, size_t size)
{
    size_t writen_size;
...
void *thread1()
{
		...
		{
			if(symbol % 2 != 0){
			int dup_page = memory_global_sync_dup_page();
			printf("dup_page:%d\n", dup_page);	
			...
			double speed = ram_list.dirty_pages;
			printf("speed:%g\n", speed);
			
			char s[16];
			sprintf(s, "%g\n", speed * 4 / 1024.0);	
			FILE *fp;
			fp = fopen("speed.dat", "a");
