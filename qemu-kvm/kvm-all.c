...
static int kvm_get_dirty_pages_log_range_end(MemoryRegionSection *section,
...
//	    printf("%lu  ", bitmap[i]);
            ...
            do {
                ...
                printf("%d\n", addr);
				memory_region_get_dirty_speed(section->mr, addr,
...
static int kvm_physical_sync_dirty_speed_end(MemoryRegionSection *section)
{
	printf("3\n");
    KVMState *s = kvm_state;
    ...
    while (start_addr < end_addr) {
		...
		//printf("%o\n", d.dirty_bitmap);
        kvm_get_dirty_pages_log_range_end(section, d.dirty_bitmap);
        start_addr = mem->start_addr + mem->memory_size;
    }
...
static void kvm_log_sync_dirty_speed_end(MemoryListener *listener,
                         MemoryRegionSection *section)
{
    int r;
	
	printf("2\n");
    r = kvm_physical_sync_dirty_speed_end(section);
    if (r < 0) {
        abort();
    }
}
...
static MemoryListener kvm_memory_listener = {
	...
	.log_sync_dirty_speed_end = kvm_log_sync_dirty_speed_end,
