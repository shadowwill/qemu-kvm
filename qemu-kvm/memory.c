...
void memory_region_get_dirty_speed(MemoryRegion *mr, target_phys_addr_t addr,
...
int get_dirtypages_amount()
{
	return get_dirty_amount();
}

void set_track_flag(int flag)
{
	set_dirty_track_flag(flag);
}
...
void memory_region_reset_dirty_all()
{
	ram_addr_t addr;
	RAMBlock *block;
    QLIST_FOREACH(block, &ram_list.blocks, next) {
        for (addr = 0; addr < block->length; addr += TARGET_PAGE_SIZE) {
            memory_region_reset_dirty(block->mr, addr, TARGET_PAGE_SIZE, DIRTY_MEMORY_MIGRATION);
        }
    }
}
...
int get_dup_page()
{
    ...
    QLIST_FOREACH(block, &ram_list.blocks, next) {
        for (addr = 0; addr < block->length; addr += TARGET_PAGE_SIZE) {
			uint8_t *p;
			mr = block->mr;
			p = memory_region_get_ram_ptr(mr) + addr;
			if (is_dup_page(p)) {
				dup_page++;
			}
		}
...
int memory_global_sync_dup_page()
{
	int dup_page = get_dup_page();
	return dup_page;
}



void  memory_global_sync_dirty_speed()
{
	memory_region_reset_dirty_all();
